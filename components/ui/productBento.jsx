...
const ProductBento = () => {
  ...
  const gradient = 'rounded-2xl bg-gradient-to-b from-slate-800 to-slate-900';
  ...
  const mobileContent = (
    <div className='mt-4 pb-16'>
      ...
      >
        <SwiperSlide>
          <div className={`relative ${gradient} p-4 h-64 w-max`}>
            <h1 className='text-3xl font-bold'>Anabolic Muscle Builder</h1>
            <p className='text-xl'>Killer deal! Buy <b className='text-red-500'>two for the price of one</b></p>
            <span className='absolute bottom-4 right-4 text-8xl text-red-500 font-black'>2X1</span>
          </div>
        </SwiperSlide>

        <SwiperSlide>
          <div className={`w-max relative ${gradient} to-slate-900 h-64 p-4 overflow-hidden flex flex-col`}>
            <img
        ...
        <SwiperSlide>
          <div className={`relative min-w-80 w-max ${gradient} to-slate-900 col-span-2 h-64 p-4 coanda grid place-content-center`}>
            <Link
              href="/products"
              className="text-3xl font-semibold group transition duration-300 animate-fade-in hover:text-red-500"
            >
              Check out more
  ...
  return (
    <div className='mt-16 cursor-pointer overflow-x-hidden'>
      <span className='md:text-heading text-4xl'>Other <span className='text-red-500'>Products</span></span>
